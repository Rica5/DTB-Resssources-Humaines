...
class AvanceList {
        ...
        this.grid = new gridjs.Grid({
            columns: [
                ...
                "Nom", "M-CODE", "Montant souhaité (MGA)", "Montant (MGA)", "Date de paiement", "Status", {
                    ...
                    name: 'Action',
                    width: '30px',
                    formatter: (id) => gridjs.html(`<button type="button" class="btn btn-sm btn-primary" style="margin: -16px;" onClick="openEditModal('${id}')"><i class="fa fa-pen"></i></button>`)
                }],
            data: data.map(d => [
    ...
    async getDemandById(id) {
        try {
            const res = await fetch(`${AvanceList.API_URL.DEMAND}/${id}`);
            const { data } = await res.json();
            return data;
        } catch (error) {
    ...
    async updateDemande(id, data) {
        try {
            const res = await fetch(`/api/avance/${id}`, {
                method: 'PUT',
...
async function openEditModal(id) {
    const demande = await av.getDemandById(id);
    if (demande) {
        // how modal (sweet alert)
        Swal.fire({
            title: 'Modifier la demande de salaire de ' + demande.user?.m_code,
            html: `
                <label for="montantDemande" style="display: block; margin-bottom: 8px;">Montant demandé :</label>
                <input type="number" id="montantDemande" class="swal2-input" value="${demande.desired_amount}" placeholder="Entrez le montant demandé" style="margin-bottom: 15px;">
                <label for="montantAccorde" style="display: block; margin-bottom: 8px;">Montant accordé :</label>
                <input type="number" id="montantAccorde" class="swal2-input" value="${demande.amount_granted}" placeholder="Entrez le montant accordé">
        `,
            ...
            preConfirm: () => {
                const montantDemande = document.getElementById('montantDemande').value;
                const montantAccorde = document.getElementById('montantAccorde').value;

                if (!montantDemande || isNaN(montantDemande) || parseFloat(montantDemande) <= 0) {
                    Swal.showValidationMessage('Veuillez entrer un montant demandé valide.');
                } else if (!montantAccorde || isNaN(montantAccorde) || parseFloat(montantAccorde) < 0) {
                    Swal.showValidationMessage('Veuillez entrer un montant accordé valide.');
                }

                return { montantDemande, montantAccorde };
            },
        }).then(async (result) => {
            if (result.isConfirmed) {
                const { montantDemande, montantAccorde } = result.value;

                // update data by requesting
                const updated = await av.updateDemande(demande._id, {
                    desired_amount: montantDemande,
                    amount_granted: montantAccorde
                });
                console.log(updated)
                if (updated.ok) {
                    Swal.fire('Succès', 'Les montants ont été enregistrés.', 'success');
                    // reload table
                    av.initGrid();
                } else {
                    Swal.fire('Failed', 'Une erreur s\'est produite!', 'error');
                }
            }
        });
    }
