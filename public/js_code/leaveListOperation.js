...
function rendu_header(active) {
  ...
  return `
    ...
    <div onclick="active_row('solde')" class="text-center mx-1 ${classes[2]}">
      SOLDE
    </div>
    <div>
...
function rendu_body(opt) {
  ...
  if (opt == "solde") {
    users.forEach(us => {
      if (search_texting_solde(us.m_code)) {
        temp_row.push(rendu_solde(us))
...
function search_texting_solde(code) {
  if (search_text.trim() != "" && row_activated != "en cours") {
    if (code.includes(search_text.toUpperCase())) {
      return true
    }
...
function edit_leave(url, type, startings, endings, val, mt, id) {
  ...
  http.onreadystatechange = function () {
    if (this.readyState == 4 && this.status == 200) {
      if (this.responseText == "Ok") {
        info.innerHTML = "Congé le " + moment(startings).format("DD/MM/YYYY") + null_val(endings, startings) + " pour " + code_selected + " modifiée";
        info.style.display = "block";
        waiting_edit.style.opacity = 0;
...
function select_delete(id, code, debut, fin, duree) {
  ...
  document.getElementById("begin").innerHTML = "Début : " + debut;
  document.getElementById("end").innerHTML = "Fin : " + fin;
